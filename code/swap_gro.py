...
def swap_gro(molA_file_name: str, molB_file_name: str):
    """
    ...
    nameA = deter_res(molA.topology, swap_map['Swap A'].to_list() + swap_map['Swap B'].to_list())
    nameB = deter_res(molB.topology, swap_map['Swap A'].to_list() + swap_map['Swap B'].to_list())
    df_atom_swap = deter_common(molA_file, molB_file, nameA, nameB)
    
    # Step 4: Fix break if present for solvated systems only
    if len(molA.topology.select('water')) != 0:
        A_dimensions = get_dimensions(molA_file)
        B_dimensions = get_dimensions(molB_file)
        molA = fix_break(molA, nameA, A_dimensions, connection_map[connection_map['Resname'] == nameA])
        molB = fix_break(molB, nameB, B_dimensions, connection_map[connection_map['Resname'] == nameB])
    ...
    if len(miss_B) != 0:
        df_atom_swap = get_miss_coord(molB, molA, nameB, nameA, df_atom_swap, 'B2A', swap_map[(swap_map['Swap A'] == nameA) & (swap_map['Swap B'] == nameB)])
    if len(miss_A) != 0:
        df_atom_swap = get_miss_coord(molA, molB, nameA, nameB, df_atom_swap, 'A2B', swap_map[(swap_map['Swap A'] == nameB) & (swap_map['Swap B'] == nameA)])
    
    #Reprint preamble text
    line_start = print_preamble(molA_file, molB_new, len(miss_B), len(miss_A))

    #Print new coordinates to file for molB
    write_new_file(df_atom_swap, 'A2B', 'B2A', line_start, molA_file, molB_new, nameA, nameB, copy.deepcopy(molA.xyz[0]), miss_A)
    
    #Print new coordinates to file 
    #Reprint preamble text
    line_start = print_preamble(molB_file, molA_new, len(miss_A), len(miss_B))
    
    #Print new coordinates for molA
    write_new_file(df_atom_swap, 'B2A', 'A2B', line_start, molB_file, molA_new, nameB, nameA, copy.deepcopy(molB.xyz[0]), miss_B)
...
if __name__ == "__main__":
    ...
    #Load input options
    parser = argparse.ArgumentParser(prog='swap_gro',description='Coordinate swapping between two gro files')
    parser.add_argument('-swap', '-s', required=True, nargs='+', type=str, help='Input gro file paths to swap')
